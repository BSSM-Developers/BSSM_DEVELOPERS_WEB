...
interface AnnouncementDevEditorProps {
  slug?: string;
  initialTitle: string;
...
const resolveErrorMessage = async (response: Response): Promise<string> => {
  const fallback = "공지사항 저장에 실패했습니다.";
...
export function AnnouncementDevEditor({
  slug,
  initialTitle,
  initialBlocks,
  mode = "edit",
}: AnnouncementDevEditorProps) {
  ...
  const handleSave = useCallback(async () => {
    ...
    try {
      setIsSaving(true);
      const endpoint = mode === "create" ? "/api/dev-content/notice" : `/api/dev-content/notice/${slug}`;
      ...
      await confirm({
        ...
        message: mode === "create" ? "공지사항이 생성되었습니다." : "공지사항이 저장되었습니다.",
        confirmText: "확인",
        hideCancel: true,
      });
      ...
      if (!nextSlug) {
        throw new Error("생성된 공지사항 slug를 확인할 수 없습니다.");
      }
      router.replace(`/announcements/${nextSlug}?mode=edit`);
      router.refresh();
    } catch (error) {
  ...
  return (
    <EditorShell>
      <EditorHeader>
        <HeaderButtons>
          <SecondaryButton type="button" onClick={() => router.push("/announcements/new")} disabled={isSaving}>
            새 공지
          </SecondaryButton>
          {mode === "edit" && slug ? (
          <SecondaryButton type="button" onClick={() => router.push(`/announcements/${slug}`)} disabled={isSaving}>
            보기 모드
          </SecondaryButton>
          ) : null}
